...
Jedoch lässt sich dieser Zeitaufwand sogar auf $\mathcal O(m)$ verringern, wie wir in den nächsten Abschnitten erkennen werden.
...
\todo{check this:}
Das kann man mit einfachem Durchlaufen aller Endpunkte in $P$ in aufsteigender Reihenfolge erreichen, also in $\mathcal O(n)$
Zeit.
...
Eine genauere Analyse dieses Verfahrens und dessen Laufzeit $\mathcal O(m)$ beschreiben \todo{Carlisle und Llyod in 'On the
...
Da wir allerdings insgesamt eine Laufzeit von $\mathcal O(m)$ zum Ziel haben, können wir das \todo{vorangegangene} Verfahren
