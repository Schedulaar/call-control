...
Nun berechnet der Algorithmus zu jedem Pfad $p \in P$ den {\em bevorzugten Anführer von $p$}, also den
größten Pfad in gieriger Ordnung, dessen Endknoten kleinergleich dem Anfangsknoten von $p$ ist.
\todo{check this:}
...
Ist $\find(q)$, also der Repräsentant der Gruppe von $q$, ein Anführer, so ist er der optimale Anführer von $p$ - ist
insbesondere $q$ gerade Anführer, so ist $q = \find(q)$ - und $p$ wird neuer Anführer der Farbe, indem $p$ gefärbt darin
gefärbt wird.
Um die neuen Erkenntnisse in der Datenstruktur festzuhalten, wird die Gruppe $\find(q)$ mit der Vorgänger-Gruppe
$\find(q')$, wobei $q'$ der Vorgänger-Pfad von $q$ in gieriger Ordnung ist.



Ist gerade ein Anführer, so ist er mit Sicherheit der optimale Anführer von $p$.
Ist $q$ kein Anführer, so wurde er entweder abgelehnt oder ist kein Anführer mehr, und wir betrachten $\find(q)$,
um
