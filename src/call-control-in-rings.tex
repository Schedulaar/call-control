...
\begin{definition}[Profil]
    Sei $Q$ eine Menge von Pfaden. Die \todo{should i outsource this to a lemma?} monoton fallende Folge
...
Wir versuchen für jedes $k \in \set N$ eine geeignete Teilmenge $Q_k \subseteq P$ mit $k$ Pfaden zu finden.
Existiert eine solche nicht, so wissen wir, dass $Q_{k-1}$ eine größte geeignete Menge, also die gesuchte Lösung, ist.
...
\begin{proof}
    ...
    Nach Lemma~\ref{lem:greedyAlgorithm} ist $G_{i}$ eine minimale Menge mit $k$ Pfaden, die auf der Kette mit den durch $\pi_{i}$
...
\begin{proof}
    ...
    zum Startprofil $\pi_i$ ist (da $\pi_i \leq \pi_{Q^*}$), gilt nach Lemma~\ref{lem:greedyAlgorithm}, dass $G_{i} \leq Q^*$.
\end{proof}

Nun können wir die Korrektheit der Prozedur vervollständigen:

\begin{lemma}
    Die Prozedur liefert die korrekte Rückgabe in höchsten $n\cdot c(e_0)$ Runden.
\end{lemma}
\begin{proof}
    Dazu unterscheiden wir, ob eine $k$-elementige geeignete Menge existiert.
    ...
    weswegen der gierige Algorithmus nach Lemma~\ref{lem:greedyAlgorithm} eine mindestens $k$-elementige Menge zurückgibt.
